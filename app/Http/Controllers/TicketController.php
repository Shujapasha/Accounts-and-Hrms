    ...
    {   
        ...
        if (\Auth::user()->can('manage ticket')) {
            ...
            if ($user->type == 'employee') {
                ...
                $tickets = Ticket::select('hrm_tickets.*')->join('users', 'hrm_tickets.created_by', '=', 'users.id')->where('users.created_by', '=', \Auth::user()->creatorId())->orWhere('hrm_tickets.created_by', \Auth::user()->creatorId())->get();
            }
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('create ticket')) {
           ...
           $date1 = date("Y-m-d");
            ...
            $resp = Utility::sendEmailTemplate('ticket_create', [$employee->email], $uArr);
